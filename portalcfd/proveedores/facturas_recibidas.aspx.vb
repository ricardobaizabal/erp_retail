...
Partial Public Class facturas_recibidas
    ...
    Private Sub MuestraLista()
        ...
        '
        Dim ObjData As New DataControl(1)
        ds = ObjData.FillDataSet("exec pFacturaProveedor @cmd=6 , @fechaini='" & calFechaInicio.SelectedDate.Value.ToShortDateString & "', @fechafin='" & calFechaFin.SelectedDate.Value.ToShortDateString & "', @proveedorid='" & proveedorid.SelectedValue.ToString & "', @sucursalid='" & sucursalid.SelectedValue.ToString & "'")
    ...
    Protected Sub facturaslist_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles facturaslist.NeedDataSource
        ...
        ds = ObjData.FillDataSet("exec pFacturaProveedor @cmd=6 , @fechaini='" & calFechaInicio.SelectedDate.Value.ToShortDateString & "', @fechafin='" & calFechaFin.SelectedDate.Value.ToShortDateString & "', @proveedorid='" & proveedorid.SelectedValue.ToString & "', @sucursalid='" & sucursalid.SelectedValue.ToString & "'")
        facturaslist.DataSource = ds
        facturaslist.MasterTableView.NoMasterRecordsText = "No se encontraron facturas"
        ObjData = Nothing
        '
        Thread.CurrentThread.CurrentCulture = CultureInfo.CreateSpecificCulture("es-MX")
        Thread.CurrentThread.CurrentUICulture = New CultureInfo("es-MX")
