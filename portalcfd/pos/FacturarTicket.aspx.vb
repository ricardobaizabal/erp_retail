...
#Region "Load Initial Values"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            objCat.CatalogoString(cmbMetodoPago, "select codigo, descripcion from tblc_MetodoPago order by descripcion", "PUE")
    ...
    Private Sub CargaCFD()
        ...
        Dim cmd As New SqlCommand("EXEC pCFD @cmd=303, @remisionId='" & Session("remisionId").ToString & "'", conn)
    ...
    Private Sub CargaCliente(ByVal ClienteId As Long)
        ...
        Try
            ...
            If rs.Read() Then
                ...
                ObjData.Catalogo(cmbFormaPago, "select id, nombre as descripcion from tblFormaPago order by nombre", rs("formapagoid"))
...
#Region "Add Invoice Items"
    ...
    Protected Sub InsertItem(ByVal id As Integer, ByVal item As GridItem)
        ...
        If ValidarConcepto = True Then
            ...
            Else
                ...
                Else
                    Dim objdata As New DataControl(1)
                    objdata.RunSQLQuery("EXEC pCFD @cmd=2, @cfdid='" & Session("CFD").ToString & "', @codigo='" & lblCodigo.Text & "', @descripcion='" & txtDescripcion.Text & "', @cantidad='" & txtQuantity.Text & "', @unidad='" & lblUnidad.Text & "', @productoid='" & id.ToString & "', @presentacionid='" & lblPresentacionId.Text.ToString & "', @descuento='" & txtDescuento.Text.ToString & "', @retencionbit='" & retencionbit.ToString & "', @tiporetencion='" & tiporetencion.ToString & "',@cuentaPredial='" & txtCuenta.Text & "'")
                    objdata = Nothing
                End If
            End If
    ...
    Private Sub DisplayItems()
        ...
        Dim ObjData As New DataControl(1)
        itemsList.MasterTableView.NoMasterRecordsText = Resources.Resource.ItemsEmptyGridMessage
        ds = ObjData.FillDataSet("EXEC pCFD @cmd=304, @remisionId='" & Session("remisionId").ToString & "'")

        For Each row As DataRow In ds.Tables(0).Rows
            Dim objdata1 As New DataControl(1)
            objdata1.RunSQLQuery("EXEC pCFD @cmd=2, @cfdid='" & Session("CFD").ToString & "', @codigo='" & row("codigo") & "', @descripcion='" & row("descripcion") & "', @cantidad='" & row("cantidad") & "', @unidad='" & row("unidad") & "', @productoid='" & row("productoid") & "', @presentacionid='" & row("presentacionid") & "', @descuento='" & row("descuento") & "', @retencionbit='" & 0 & "', @tiporetencion='" & 0 & "',@cuentaPredial='" & 0 & "'")
            objdata1 = Nothing
        Next

        Dim ds1 As DataSet
        ds1 = ObjData.FillDataSet("EXEC pCFD @cmd=3, @cfdid='" & Session("CFD").ToString & "'")

        itemsList.DataSource = ds1
        itemsList.DataBind()
        ObjData = Nothing
    ...
    Protected Sub itemsList_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles itemsList.NeedDataSource
        Dim conn As New SqlConnection(Session("conexion"))
        Dim cmd As New SqlDataAdapter("EXEC pCFD @cmd=304, @cfdid='" & Request("id") & "'", conn)
...
#Region "Create Invoice"
    ...
    Protected Sub btnCreateInvoice_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreateInvoice.Click
        If Page.IsValid Then
            ...
            '
            If cmbDocumento.SelectedValue > 6 Then
        ...
        Try
            ...
            While rs.Read
                ...
                If rs("descuento") > 0 Then
                    ...
                    Traslado.SetAttribute("Base", Math.Round(rs("base_iva") - rs("descuento"), 6))
                Else
                ...
                If CBool(rs("exento")) = False Then
                    ...
                    Traslado.SetAttribute("Importe", Math.Round(rs("iva"), 2))
                ...
                If ieps >= 1 Then
                    ...
                    If rs("descuento") > 0 Then
                        ...
                        Traslado.SetAttribute("Base", Math.Round(Base, 6))
                    Else
                        Base = Math.Round(rs("importe"), 6)
                        Traslado.SetAttribute("Base", Math.Round(Base, 6))
                    End If
    ...
    Private Sub cfdtimbrado()
        Dim Objdata As New DataControl(1)
        Objdata.RunSQLQuery("exec pCFD @cmd=24, @uuid='" & UUID.ToString & "', @cfdid='" & Session("CFD").ToString & "', @remisionId='" & Session("remisionId").ToString & "'")
        Objdata = Nothing
    End Sub
...
#Region "Manejo de PDF"
    ...
    '    reporte.ReportParameters("txtFechaEmision").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "Fecha", "cfdi:Comprobante")
    '    reporte.ReportParameters("txtFechaCertificacion").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "FechaTimbrado", "tfd:TimbreFiscalDigital")
    '    reporte.ReportParameters("txtUUID").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "UUID", "tfd:TimbreFiscalDigital")
    '    reporte.ReportParameters("txtPACCertifico").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "RfcProvCertif", "tfd:TimbreFiscalDigital")
    '    reporte.ReportParameters("txtSerieEmisor").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "NoCertificado", "cfdi:Comprobante")
    '    reporte.ReportParameters("txtSerieCertificadoSAT").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "NoCertificadoSAT", "tfd:TimbreFiscalDigital")
    '    reporte.ReportParameters("txtClienteRazonSocial").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "Nombre", "cfdi:Receptor")
    '    reporte.ReportParameters("txtClienteCalleNum").Value = callenum
    '    reporte.ReportParameters("txtClienteColonia").Value = colonia
    '    reporte.ReportParameters("txtClienteCiudadEstado").Value = ciudad
    '    reporte.ReportParameters("txtClienteRFC").Value = "R.F.C. " & GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "Rfc", "cfdi:Receptor")        '
    '    reporte.ReportParameters("txtSelloDigitalCFDI").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "Sello", "cfdi:Comprobante")
    '    reporte.ReportParameters("txtSelloDigitalSAT").Value = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "SelloSAT", "tfd:TimbreFiscalDigital")
    '    reporte.ReportParameters("txtInstrucciones").Value = instrucciones
    '    reporte.ReportParameters("txtCantidadLetra").Value = CantidadTexto
    '    reporte.ReportParameters("txtSubtotal").Value = FormatCurrency(importe, 2).ToString
    '    reporte.ReportParameters("txtTasaCero").Value = FormatCurrency(importetasacero, 2).ToString
    '    reporte.ReportParameters("txtEtiquetaIVA").Value = "IVA 16%"
    '    tipo_comprobante = GetXmlAttribute(Server.MapPath("~/clientes/" + Session("appkey").ToString + "/xml") & "\link_" & serie.ToString & folio.ToString & "_timbrado.xml", "TipoDeComprobante", "cfdi:Comprobante")
    '    If tipo_comprobante = "I" Then
    '        tipo_comprobante = "I - Ingreso"
    '    ElseIf tipo_comprobante = "E" Then
    '        tipo_comprobante = "E - Egreso"
    ...
    '    '        Return reporte
    '    '    Case Else
    '    '        Dim reporte As New Formatos.formato_cfdi33_link
    '    '        reporte.ReportParameters("plantillaId").Value = plantillaid
    '    '        reporte.ReportParameters("cfdiId").Value = cfdid
    '    '        Select Case tipoid
    '    '            Case 1, 4, 7
